...
for(const art of artworks){calls=[];const blob=await exportReaction(art,art.caption);const bytes=Buffer.from(await blob.arrayBuffer());assert.equal(bytes[0],0xff);assert.equal(bytes[1],0xd8);const image=await loadImage(bytes);assert.equal(image.width,1600);assert.equal(image.height,900);assert(calls.some(t=>t.includes(art.artist)));assert(calls.some(t=>t.includes('mood.jezamancenido.com')));assert(calls.includes(`MUSEUM MOOD / ${art.label.toUpperCase()}`));await fs.writeFile(`outputs/${art.id}.jpg`,bytes);console.log(`PASS export: ${art.id}`)}
