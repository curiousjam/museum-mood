...
export default function Home(){
 ...
 return <main style={{'--accent':art.accent} as CSSProperties}>
  <header className="museum-header"><a className="wordmark" href="/" aria-label="Museum Mood home"><Landmark size={18}/><b>MUSEUM MOOD</b></a><span>Old art. Current feelings.</span><a className="source-link" href="https://www.metmuseum.org/hubs/open-access" target="_blank" rel="noreferrer">The open-access collection <ArrowUpRight size={14}/></a></header>
  <div className="gallery-shell">
   <section className="canvas-stage" aria-label="Museum mood gallery">
    <div className="canvas-intro"><p className="eyebrow">THE OPEN ACCESS REACTION COLLECTION</p><h1>Find your feeling<br/>in the <em>collection.</em></h1><p>Eight very old paintings.<br/>Eight very current moods.</p></div>
    <RadioGroup value={mood} onValueChange={v=>selectMood(v)} className="mood-rail" aria-label="Choose your mood">{artworks.map((a,i)=><label key={a.id} className={`rail-item ${mood===a.id?'selected':''}`} title={a.label}><RadioGroupItem value={a.id} className="mood-radio"/><img src={a.image} alt=""/><span>0{i+1}</span></label>)}</RadioGroup>
    <div className="constellation" aria-hidden="true"><svg viewBox="0 0 1000 680" preserveAspectRatio="none"><path d="M120 245 L265 165 L430 245 L600 160 L810 238"/><path d="M265 165 L335 425 L535 510 L730 410 L810 238"/><path d="M120 245 L335 425 M430 245 L535 510 M600 160 L730 410"/></svg>{artworks.map((a,i)=><button key={a.id} tabIndex={-1} className={`art-node node-${i+1} ${mood===a.id?'active':''}`} onClick={()=>selectMood(a.id)}><img src={a.image} alt=""/><span>{a.label}</span></button>)}</div>
    <article className="focus-work" aria-label="Your reaction"><div className="frame-top"><span>REACTION NO. 0{artworks.indexOf(art)+1}</span><span className="mood-tag">{art.label}</span></div><div className="artframe" aria-busy={!ready&&!failed}>
     ...
     <p className="preview-caption" style={{fontSize:caption.length>100?'clamp(15px,1.5vw,20px)':undefined}}>{caption||'\u00a0'}</p><div className="card-footer"><span>{art.label.toUpperCase()}</span><span>MUSEUM MOOD</span></div></div>
     <div className="art-credit"><p><span>{art.title}</span> · {art.artist}, {art.date}</p><a href={art.source} target="_blank" rel="noreferrer">The Met <ArrowUpRight size={12}/></a></div>
    </article>
   </section>
   <aside className="studio-panel">
    <div className="panel-kicker"><span className="small-dot"/> PUBLIC DOMAIN · FREE TO REMIX</div>
    <div className="panel-title"><p className="eyebrow">01 / PICK A FEELING</p><h2>How are we feeling?</h2><p>Choose a painting, then make its reaction your own.</p></div>
    <RadioGroup value={mood} onValueChange={v=>selectMood(v)} className="moods" aria-label="Choose your mood">{artworks.map((a,i)=><label key={a.id} className={`mood ${mood===a.id?'selected':''}`}><RadioGroupItem value={a.id} className="mood-radio"/><span className="mood-number" aria-hidden="true">0{i+1}</span><span>{a.label}</span>{mood===a.id&&<Check size={13} className="mood-check" aria-hidden="true"/>}</label>)}</RadioGroup>
    <div className="caption-heading"><label htmlFor="caption">02 / MAKE IT YOURS</label><Button variant="ghost" className="reset" onClick={()=>setCaptions(c=>({...c,[mood]:art.caption}))} disabled={caption===art.caption}><RotateCcw size={12}/>Reset</Button></div>
    <Textarea id="caption" className="caption-input" value={caption} maxLength={180} rows={3} onChange={e=>setCaptions(c=>({...c,[mood]:e.target.value}))} aria-describedby="caption-hint"/><p id="caption-hint" className="caption-hint"><span>Your words. Their brushwork.</span><span>{caption.length}/180</span></p>
    <div className="actions"><Button className="download" onClick={()=>handleExport()} disabled={busy||!ready||failed}><ArrowDownToLine size={17}/>{busy?'Preparing…':'Download reaction'}</Button><Button variant="outline" className="share" onClick={()=>handleExport(true)} disabled={busy||!ready||failed}>Share on X <ArrowUpRight size={16}/></Button></div>
    <p className="share-note">Free 1600 × 900 image. Attach the download when sharing.</p>
    ...
    <footer><span>Human feelings. Museum quality.</span><span>Artwork from The Met.</span></footer>
   </aside>
  </div>
 </main>
}
